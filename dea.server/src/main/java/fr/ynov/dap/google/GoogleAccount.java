...
public class GoogleAccount extends Services
{
	//TODO dea by Djer Séparation Service/Controller serait mieux. Pour cette classe c'est un peu plus compliqué. 
	// A defaut cette classe étant un controller elle devrait être dans le package "web".
  /**
   ...
   * @throws IOException
   */
  GoogleAccount() throws GeneralSecurityException, IOException
  {
    super();
      ...
      final HttpSession session) throws ServletException, GeneralSecurityException
  {
    ...
    //TODO dea by Djer la config ici n'était pas indispensable, tu peux faire le choix de ne pas l'utiliser.
    // Dans ce cas penses à supprimer le code correspondant dans la Classe Config (getter/setter/....)
    final String redirectUri = buildRedirectUri(request, "/oAuth2Callback");

    final String userId = getUserid(session);
    try
    {
    	//TODO dea by Djer pourquoi passer "" en paramètre ? 
      final GoogleAuthorizationCodeFlow flow = super.getFlow("");
      final TokenResponse response = flow.newTokenRequest(decodedCode).setRedirectUri(redirectUri).execute();

      final Credential credential = flow.createAndStoreCredential(response, userId).setExpiresInSeconds(null);
    ...
    //TODO dea by Djer Es-tu sure d'avoir un Mapping sur / ?
    return "redirect:/";
  }
