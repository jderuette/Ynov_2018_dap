...
public class GoogleAccount extends ConnexionGoogle {
  ...
  public String oAuthCallback(@RequestParam final String code,
    ...
    throws ServletException, GeneralSecurityException, IOException {
    final String           decodedCode   = extracCode(request);
    final String           redirectUri   = buildRedirectUri(request,
      config.getoAuth2CallbackUrl());
    ...
    try {
      final GoogleAuthorizationCodeFlow flow       = super.getFlow(
        httpTransport);
      final TokenResponse               response   = flow
        .newTokenRequest(decodedCode).setRedirectUri(redirectUri).execute();
      final Credential                  credential = flow
        .createAndStoreCredential(response, userId);
      ...
      throw new ServletException(
        "Error while trying to conenct Google Account");
    }
    return "redirect:/" + config.getoAuth2CallbackUrl();
  }
  /**
  ...
  private String extracCode(final HttpServletRequest request)
    ...
    if (null != responseUrl.getError()) {
      LOG.error(
        "Error when trying to add Google acocunt : " + responseUrl.getError());
    ...
    try {
      ...
      if (credential != null && credential.getAccessToken() != null) {
        ...
        authorizationUrl.setRedirectUri(
          buildRedirectUri(request, config.getoAuth2CallbackUrl()));
