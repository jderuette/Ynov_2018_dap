...
public interface OutlookService {
	...
	@GET("/api/v2.0/me")
	Call<MicrosoftUser> getCurrentUser();

	@GET("/api/v2.0/me/mailfolders/{folderid}/messages")
	Call<PagedResult<MicrosoftMessage>> getMessagesUnread();

	
	@GET("/api/v2.0/me/messages?$count=true")
	Call<PagedResult<MicrosoftMessage>> getMessages(
	...
	Call<PagedResult<MicrosoftEvent>> getEvents(
			@Query("$orderby") String orderBy,
			@Query("$select") String select,
			@Query("$top") Integer maxResults);
