...
@Controller
//TODO baa by Djer javadoc de la classe ?
public class GoogleAccount extends GoogleService {
	...
	final String redirectUri = buildRedirectUri(request, maConfig.getoAuth2CallbackUrl());
	...
	try {
		...
		if (null != credential && null != credential.getAccessToken()) {
		    //TODO baa by Djer un seul caractère d'affiché c'est un peu court ! 
		    LOG.debug("New user credential stored with userId : " + userId + "partial AccessToken : "
			    + credential.getAccessToken().substring(1, 2 /*"SENSIBLE_DATA_FIRST_CHAR, SENSIBLE_DATA_LAST_CHAR*/));
		}
	    }
	} catch (IOException e) {
	...
	//TODO baa by Djer il y a-t-il un requestMapping sur / ??
	return "redirect:/";
    }
    
    //TODO commentaire faux, c'est un singleton car Spring (@Controller) en fait un singleton,
    // pas grace à se constructeur  (qui au contraire autorise la création de mutiples instances)!
    /**
	...
	try {
		...
		authorizationUrl.setRedirectUri(buildRedirectUri(request, maConfig.getoAuth2CallbackUrl()));
